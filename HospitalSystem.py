        ...
        for i in range(len(g.nodes)):
            ...
            if len(self.Agents[n]) > 0:
                ...
                for agent in self.Agents[n]:
                    if new_time + self.sys_time < agent['endTime']:
                        old_time = agent['endTime'] - self.sys_time         # 남은시간
                        interval_time = old_time - new_time
                        m = int(interval_time * N_T) * self.hospScale[n]
                        patients = max(g.paths[i].n - int(new_time * N_T)*self.hospScale[n], 0)   #hospitals cure patients
                        m = 0 if patients > m else m - patients            #moving patients
                        intersectList[n] = m
                        break
                    else:
        ...
        for i in range(len(ps)):
            if i < len(ps) - 1:
                ...
                if m > plus_ms:
                    ...
                    ps[i].m += vs[i]
                    if ps[i].ind in intersectList:
                        if ps[i].m > intersectList[ps[i].ind]:
